...
- **题目可直接维护：** 在设置中打开题库题目编辑器，修改题号、题型、考点、题干、选项、答案和文字解析，并新增、替换或删除题图与解析图。
- **PDF 直接截题：** 导入 PDF 或图片后可切换页面、裁剪、旋转和矩形填充，也能粘贴剪贴板截图；保存后同步写回本地题库。
- **设置更集中：** 学习轮次、考试日期、题库数据、导入导出和版本信息按类别收纳到统一面板，并新增学习时保持屏幕常亮的选项。
- **题库继续扩充：** 新增 184 道题的“Kira线代强化”，并修订数学与机械专业课题图；默认题库更新为 22 个题库、7252 道题。
...
- **复习弹窗更聚焦：** 移除复习模块底部重复的掌握情况按钮，上一题、题号和下一题导航居中显示。
- **题库继续扩充：** 新增“机械设计·讲义课后习题”题库，默认题库更新为 22 个题库、7139 道题。
- **发布包保持拆分：** 软件包与题库数据包继续在同一个 Release 中分别提供，便于独立更新。

## v0.3.5 复习记录与手写工具优化

- **复习记录可维护：** 可以删除单次复习记录，系统会自动重连后续记录的前置状态；重置和删除操作均需确认。
- **手写工具更安全：** 清空手写笔记、删除选中笔迹和草稿内容前增加确认，套索选择支持删除和移动。
- **交互更顺滑：** 题目切换后自动滚动到新题卡，学习看板明确区分“新题”和“复习”的每日进度。
- **题库内容修订：** 修正部分英语真题答案、机械题图与解析资源，默认题库更新为 21 个题库、7068 道题。
- **发布包拆分：** 软件与题库数据分别提供 Release，允许只更新应用或只更新题库。
...
- **题库更完整：** 默认提供 21 个题库、7068 道题，覆盖数学二、2004–2026 年英语一真题和机械专业课。
...
- **发布体验统一：** Release、软件包和题库数据包采用一致英文名称，避免中文路径或自动生成源码包造成的启动困扰。
