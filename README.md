...
如果图片放在 `01 行列式 1-基础.assets` 文件夹中，还会自动把章节命名为“行列式”，小节命名为“基础”。Q 表示题目，A 表示答案；点号后的分片序号没有固定上限。旧格式 `01-1-01.png` 继续兼容并按题目图处理。

## 数据说明

### 题库文件夹工作区（推荐）

使用最新版 Chrome 或 Edge，点击顶部“文件夹”，选择一个本地目录并授权读写。应用会扫描每个一级子文件夹作为一个题库，并将网页中的题库结构、重命名和学习标注实时写入根目录的 `题库数据.json`。

项目已经自带 [`题库工作区`](./题库工作区) 以及“高等数学 · 基础篇”“线性代数 · 强化篇”两个默认题库目录。首次使用时直接选择这个目录即可，无需手动创建文件夹。

```text
题库工作区/
├── 题库数据.json
├── 高等数学基础篇/
│   └── 01 函数极限 1-选择题.assets/
│       ├── Q-01-1-01.png
│       └── A-01-1-01.png
└── 线性代数强化篇/
    └── 01 行列式 1-基础.assets/
...
- 将新图片复制进工作区后，点击顶部“已连接”重新扫描即可导入；网页内的标注、重命名、重置等修改会自动写回清单。
