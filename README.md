...
设置 → 同步可以把学习轮次、熟练度、复习记录、考试日期、题目笔记和个人笔记同步到 OneDrive，供多台设备使用。当前实现只适配 OneDrive 个人账号，通过 Microsoft Graph 的 App Folder 保存数据；首次登录需要准备一个 Microsoft Entra 的 SPA 应用，并授予委托权限 `Files.ReadWrite.AppFolder`。

配置步骤：

1. 在 Microsoft Entra 应用注册中添加 SPA 重定向地址。设置页面会显示当前应用的重定向地址，必须与注册值完全一致。
2. 将应用的客户端 ID 填入设置，然后点击“登录 OneDrive”。
3. 登录后点击“立即同步”；另一台设备使用相同的客户端 ID 登录同一个 Microsoft 账号即可读取数据。

当前版本默认只同步用户数据和笔记；勾选“同步题库结构”后还会同步题库 JSON，但不会上传题库图片。浏览器会话关闭后需要重新登录，应用不会在后台持续同步。题库图片的增量同步和大文件断点上传将在后续版本加入。
